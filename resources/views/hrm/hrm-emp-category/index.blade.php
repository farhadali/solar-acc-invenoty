      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12" style="display: flex;">
            <a class="m-0 _page_name" href="{{ route('hrm-emp-category.index') }}">{!! $page_name ?? '' !!} </a>
            <ol class="breadcrumb float-sm-right ml-2">
               @can('hrm-emp-category-create')
             <li class="breadcrumb-item active">
                <a type="button" 
               class="btn btn-sm btn-info" 
              
               href="{{ route('hrm-emp-category.create') }}">
                     ...
                     <tbody>
                        ...
                        <tr>
                             <td style="display: flex;">
                              @can('hrm-emp-category-delete')
                                 {!! Form::open(['method' => 'DELETE','route' => ['hrm-emp-category.destroy', $data->id],'style'=>'display:inline']) !!}
                                      <button onclick="return confirm('Are you sure?')" type="submit" class="btn btn-sm btn-default"><i class="fa fa-trash _required"></i></button>
                                  {!! Form::close() !!}
                               @endcan 
                              <a  type="button" 
                                  href="{{ route('hrm-emp-category.show',$data->id) }}"
                                  class="btn btn-sm btn-default  mr-1"><i class="fa fa-eye"></i></a>

                             @can('hrm-emp-category-edit')
                                  <a  type="button" 
                                  href="{{ route('hrm-emp-category.edit',$data->id) }}"
