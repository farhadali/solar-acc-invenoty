...
<div class="_report_button_header">
 <a class="nav-link"  href="{{url('w-item-receive-from-supp')}}" role="button"><i class="fa fa-arrow-left"></i></a>
       ...
       <table class="table table-striped _grid_table">
        ...
        <tbody>
          <tr>
            ...
            <td>{{ $data->_ind_repl_in_item->_items->_name ?? '' }}</td>
            <td>{{ $data->_ind_repl_in_item->_barcode ?? '' }}</td>
            <td>{{ _report_amount($data->_ind_repl_in_item->_in_payment_amount ?? 0) }}</td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
            <td colspan="7"><b>Remarks:</b> {{$data->_ind_repl_in_item->_short_note ?? '' }} </td>
          </tr>
