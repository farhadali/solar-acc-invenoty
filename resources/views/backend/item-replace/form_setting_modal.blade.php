...
<div class="form-group row">
      ...
      <div class="form-group row">
        <label for="_default_warranty_charge" class="col-sm-5 col-form-label">Warrenty Service Charge</label>
        <select class="form-control col-sm-7" name="_default_warranty_charge">
          @foreach($p_accounts as $account)
          <option value="{{$account->id}}" @if(isset($form_settings->_default_warranty_charge))@if($form_settings->_default_warranty_charge==$account->id) selected @endif @endif>{{ $account->_name ?? '' }}</option>
          @endforeach
        </select>
      </div>
      <div class="form-group row">
